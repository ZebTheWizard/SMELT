...
def get_tweets(query=None, search_term=None, user_name=None, pages=25):
    ...
    if not incompatibleSearch:

        after_part = (
            f"include_available_features=1&include_entities=1&include_new_items_bar=true"
        )
        #here we will keep the existing query bits to ensure any old api calls
        #dependent on the software will continue to function...
        if query:
            if query.startswith("#"):
                query = quote(query)
        ...
        #Here we will integrate the new general search functionality
